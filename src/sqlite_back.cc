...
#define CYCLUS_BINDVAL(D, T) \
  ...
  CYCLUS_BINDVAL(
      MAP_VECTOR_MAP_STRING_MAP_STRING_DOUBLE,
      std::map<std::vector CYCLUS_COMMA 
      std::map<std::string CYCLUS_COMMA 
      std::map<std::string CYCLUS_COMMA double> > >);
...
#define CYCLUS_LOADVAL(D, T) \
  ...
  CYCLUS_LOADVAL(
      MAP_VECTOR_MAP_STRING_MAP_STRING_DOUBLE,
      std::map<std::vector CYCLUS_COMMA std::map<std::string CYCLUS_COMMA std::map<std::string CYCLUS_COMMA double> > >);
...
DbTypes SqliteBack::Type(boost::spirit::hold_any v) {
  if (type_map.size() == 0) {
    ...
    type_map[&typeid(std::map<std::vector, std::map<std::string, std::map<std::string,double> > >)] =
        MAP_VECTOR_MAP_STRING_MAP_STRING_DOUBLE;
  }
